            ...
            <div class="col-12">
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="text-center flex-grow-1">
                        <h1>Formulário</h1>
                        <h3 class="subtitle"><small>Adicione as informações do filme</small></h3>
                    </div>
                </div>
            </div>
        </div>
        <div class="row justify-content-start mt-4">
            <div class="col">
                <form method="POST" action="/adicionar" enctype="multipart/form-data">
                    ...
                    <input type="text" class="form-control custom-input" name="titulo" required>
                    <br><br>
                    <img src="#" width="110" height="150" class="img-responsive d-none" id="imgpreview">
                    <br><br>
                    <label for="imageinput" class="form-label">Imagem</label>
                    <input type="file" class="form-control-file" name="image" id="image" required>
                    <br><br>
                    <label for="descricaoinput" class="form-label">Descrição</label>
                    <input type="text" class="form-control custom-input" name="descricao" required>
                    <br><br>
                    <label for="videoinput" class="form-label">Vídeo</label>
                    <input type="text" class="form-control custom-input" name="video" required>
                    <br>
            </div>
            <br>
        </div>
    ...
    <script>
        ...
        file.addEventListener('change', () => {
            if (file.files.length <= 0) {
                photo.classList.add('d-none'); // Oculta a imagem se nenhum arquivo for selecionado
                return;
            }
            let reader = new FileReader();
            reader.onload = () => {
                photo.src = reader.result;
                photo.classList.remove('d-none'); // Mostra a imagem quando um arquivo é selecionado
            }
